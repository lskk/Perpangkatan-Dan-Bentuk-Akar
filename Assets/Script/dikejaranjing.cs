...
public class dikejaranjing : MonoBehaviour {
    ...
    void Start()
    {
        if (target == null)
        {
            target = GameObject.FindGameObjectWithTag("farms");
    ...
    {
        if (other.gameObject.tag == "farms" && !isDead)
        {
            // rb.velocity = Vector2.zero;
            // lives = 0;
            //     anim.SetBool("Dies", true);
        ...
        else if (other.gameObject.tag == "farms")
        {
            //   lives--;
            //   anim.SetBool("Immune", true);
            //   isImmune = true;
