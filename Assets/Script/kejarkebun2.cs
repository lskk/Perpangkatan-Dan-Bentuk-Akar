...
public class kejarkebun2 : MonoBehaviour {
    ...
    void Start()
    {
        if (target == null)
        {
            target = GameObject.FindGameObjectWithTag("kebun");
    ...
    {
        if (other.gameObject.tag == "kebun" && !isDead)
        {
            // rb.velocity = Vector2.zero;
            // lives = 0;
            //     anim.SetBool("Dies", true);
        ...
        else if (other.gameObject.tag == "kebun")
        {
            //   lives--;
            //   anim.SetBool("Immune", true);
            //   isImmune = true;
